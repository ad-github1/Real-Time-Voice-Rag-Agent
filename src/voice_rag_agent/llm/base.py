...
class TemplateReasoner:
    ...
    def _answer(self, query: str, chunks: list[RetrievedChunk]) -> str:
        ...
        if any(term in lower_query for term in ("latency", "fast", "real-time", "realtime")):
            lead = (
                "The agent keeps latency low by streaming each stage and caching retrieval results."
            )
        elif any(
            term in lower_query
            ...
            lead = (
                "The protocol layer exposes the same grounded query capability through MCP and A2A."
            )
        else:
