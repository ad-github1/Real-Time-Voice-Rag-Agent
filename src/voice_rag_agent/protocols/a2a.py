...
def create_a2a_app(settings: Settings | None = None) -> Any:
    ...
    class Message(BaseModel):  # type: ignore[no-redef]
        role: str
        parts: list[dict[str, Any]]

    class TaskRequest(BaseModel):  # type: ignore[no-redef]
        id: str | None = None
        message: Message
    ...
    async def send_subscribe(request: TaskRequest) -> StreamingResponse:
        ...
        async def body() -> Any:
            async for line in async_events_to_ndjson(engine.stream_query(text, trace_id=request.id)):
                yield line
